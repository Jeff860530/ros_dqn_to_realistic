...
float serialRead_L = 0;
float serialRead_R = 0;
...
Motor m1(X_STEP_PIN,X_DIR_PIN,StepPerRev);
Motor m2(Y_STEP_PIN,Y_DIR_PIN,StepPerRev);
...
void loop() {
  ...
  m1.go(abs(serialRead_L),(serialRead_L>0)?(true):(false));
  m2.go(abs(serialRead_R),(serialRead_R>0)?(true):(false));
}
